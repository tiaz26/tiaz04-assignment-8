...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.contour(xx, yy, Z, levels=[0.5], colors='green', linestyles='--', label='Decision Boundary')  # Decision boundary
        ...
        if i == 1:
            plt.legend(loc='lower right', fontsize=20, handles=[
            plt.Line2D([0], [0], color='blue', marker='o', linestyle='None', label='Class 0'),
            plt.Line2D([0], [0], color='red', marker='o', linestyle='None', label='Class 1'),
            plt.Line2D([0], [0], color='green', linestyle='--', label='Decision Boundary')
    ])
    ...
    plt.subplot(3, 3, 2)
    plt.plot(shift_distances, beta1_list, marker='o', linestyle='-', color='red')
    ...
    plt.subplot(3, 3, 4)
    slope_ratios = [b1 / b2 if b2 != 0 else np.inf for b1, b2 in zip(beta1_list, beta2_list)]
    plt.plot(shift_distances, slope_ratios, marker='o', linestyle='-', color='purple')
    ...
    plt.subplot(3, 3, 5)
    intercept_ratios = [b0 / b2 if b2 != 0 else np.inf for b0, b2 in zip(beta0_list, beta2_list)]
    plt.plot(shift_distances, intercept_ratios, marker='o', linestyle='-', color='orange')
    ...
    # Plot logistic loss
    plt.subplot(3, 3, 6)
    plt.plot(shift_distances, loss_list, marker='o', linestyle='-', color='cyan')
    ...
    plt.subplot(3, 3, 7)
    plt.plot(shift_distances, margin_widths, marker='o', linestyle='-', color='magenta')
    plt.title("Shift Distance vs Margin Width")
    plt.xlabel("Shift Distance")
    plt.ylabel("Margin Width")

    # Hide the remaining subplots if any
    for j in range(8, 10):
        plt.subplot(3, 3, j)
        plt.axis('off')
